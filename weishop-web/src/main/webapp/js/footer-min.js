...
function topc() {
    $("#gotopc").click(function () {
        var b = $(this).attr("res-cd");
        document.cookie = "terminal_flag=0;domain=" + (b ? b : ".suning.com") + ";path=/";
        var a = $(this).attr("res-url");
        if (a) {
            window.location.href = a
        } else {
            window.location.href = "http://www.suning.com/?utm_source=suning&utm_medium=pc"
        }
    })
}
function logon() {
...
function register() {
    $("#footerRegister").click(function () {
        window.location.href = "http://" + sn.mtsDomain + "/wap/register/getRegister.do"
    })
}
function logonOff() {
...
function cart() {
    $("#shopCartTip").click(function () {
        window.location.href = "http://" + sn.mtsDomain + "/cart1/gateway/cart1View.do"
    })
}
function userEvent() {
    $("#footerUserName").click(function () {
        window.location.href = sn.mobileBase + "/" + sn.wapMtsWeb + "/accountcenter/private/getAccountCenter.do"
    })
}
...
function checkMergeCart() {
    probeAuthStatus(function () {
        var a = $.cookie("mergeFlag");
...
function mergeCart() {
    ...
    $.ajax({
        type: "GET",
        url: "http://" + sn.mainHostDomain + "/webapp/wcs/stores/servlet/SNMobileShoppingCartQuery",
        cache: false,
        async: false,
        dataType: "jsonp",
        data: a,
...
function openClient() {
    var e = "";
    var b = "";
    if (sn.supplierCode && sn.supplierCode.length > 0) {
    ...
    if (navigator.userAgent.match(/(iPhone|iPod|iPad);?/i)) {
        ...
        if (navigator.userAgent.match(/android/i)) {
            ...
            setTimeout(function () {
                if (d) {
                    window.location.href = "http://mapp.suning.com/a.php?s=qrcode/offline&f=sjyzx&pack=com.suning.mobile.ebuy"
                }
            }, 5000)
        }
    }
